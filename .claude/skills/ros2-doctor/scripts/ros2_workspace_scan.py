    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for dirname in dirnames:
            if dirname in SKIP_DIRS and not (
                include_build_artifacts and current == root and dirname in {"build", "install", "log"}
            ):
                continue
            kept.append(dirname)
...
def parse_package_xml(path: Path) -> tuple[str, str]:
    ...
    try:
        ...
        build_match = re.search(
            r"<build_type>\s*([^<]+?)\s*</build_type>", text, re.IGNORECASE
        )
        return (
...
def find_python_package_dirs(package_dir: Path) -> tuple[Path, ...]:
    ...
    for setup_file in (package_dir / "setup.py", package_dir / "setup.cfg", package_dir / "pyproject.toml"):
...
def collect_packages(root: Path) -> tuple[list[Package], list[str]]:
    ...
    for package_xml in package_xmls:
        ...
        if has_setup_py:
            parsed, setup_resource_marker, setup_console_scripts = parse_setup_py(package_dir / "setup.py")
        ...
        if not has_setup_py and not has_cmakelists:
            ...
            issues.append(f"`{rel(package_xml, root)}` has no `setup.py` or `CMakeLists.txt` nearby.{suffix}")
        ...
        if has_setup_py and not setup_resource_marker:
            issues.append(f"`{rel(package_dir / 'setup.py', root)}` may be missing the ament resource marker in `data_files`.")
        if has_setup_py and not setup_console_scripts:
            issues.append(f"`{rel(package_dir / 'setup.py', root)}` has no detected `console_scripts` entry point; this matters only if the package should expose nodes with `ros2 run`.")
        for module_dir in find_python_package_dirs(package_dir):
            if not (module_dir / "__init__.py").exists():
                issues.append(f"`{rel(module_dir, root)}` looks like a Python package dir but lacks `__init__.py`.")
...
def collect_by_kind(root: Path) -> dict[str, list[Path]]:
    ...
    for file_path in iter_files(root, include_build_artifacts=True):
        ...
        if file_path.suffix in {".msg", ".srv", ".action"} and any(part in {"msg", "srv", "action"} for part in parts):
...
def find_distro_hints(root: Path) -> dict[str, list[Path]]:
    ...
    return {distro: sorted(paths, key=lambda path: rel(path, root)) for distro, paths in hints.items() if paths}
...
def collect_launch_refs(root: Path, launch_files: list[Path]) -> list[LaunchRef]:
    ...
    return sorted(set(refs), key=lambda item: (rel(item.launch_file, root), item.package, item.executable))
...
def launch_reference_issues(root: Path, packages: list[Package], refs: list[LaunchRef]) -> list[str]:
...
def artifact_issues(root: Path) -> list[str]:
    ...
    newest_source = max((path.stat().st_mtime for path in src_files if safe_stat(path)), default=None)
    for dirname in ("build", "install", "log"):
        ...
        if newest_source and stat.st_mtime < newest_source:
            issues.append(f"`{dirname}/` exists ({child_count} entries, modified {stamp}) and may be older than source metadata.")
        else:
            issues.append(f"`{dirname}/` exists ({child_count} entries, modified {stamp}); verify it is not stale before debugging.")
    return issues
...
def command_status() -> list[str]:
    ...
    for command in ("ros2", "colcon", "vcs"):
        ...
        rows.append(f"`{command}`: {'available at `' + found + '`' if found else 'not found on PATH'}")
    return rows
...
def main() -> int:
    ...
    if packages:
        ...
        for build_type, count in sorted(build_counts.items()):
            names = ", ".join(f"`{package.name}`" for package in packages if package.build_type == build_type)
    ...
    if not src.exists():
        suggested.insert(0, "Create `src/` only if this directory is intended to be a ROS 2 workspace.")
